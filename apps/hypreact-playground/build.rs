...
fn main() {
    ...
    let template_root = workspace_root.join("examples/template");
    let out_dir = PathBuf::from(std::env::var("OUT_DIR").expect("out dir"));

    println!("cargo:rerun-if-changed={}", template_root.display());

    let specs = collect_specs(&template_root);
    let generated = render_manifest(&specs);
    fs::write(out_dir.join("editor_files_manifest.rs"), generated)
        .expect("write editor files manifest");
}

fn collect_specs(template_root: &Path) -> Vec<FileSpec> {
    let mut specs = Vec::new();

    let config_relative = ["config.ts", "config.tsx"]
        .into_iter()
        .find(|candidate| template_root.join(candidate).is_file())
        .unwrap_or_else(|| panic!("examples/template must contain config.ts or config.tsx"));
...
fn build_spec_with_language(
    ...
    FileSpec {
        ...
        is_layout_source: relative.starts_with("layouts/")
            && matches!(language, "typescript" | "typescriptreact"),
    }
}
...
fn render_manifest(specs: &[FileSpec]) -> String {
    ...
    let enum_variants =
        specs.iter().map(|spec| format!("    {},", spec.variant)).collect::<Vec<_>>().join("\n");
