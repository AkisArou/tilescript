...
fn collect_specs(language: AuthoringLanguage, template_root: &Path) -> Vec<FileSpec> {
    ...
    if matches!(language, AuthoringLanguage::Lua | AuthoringLanguage::Fennel) {
        specs.push(FileSpec {
            variant: "SdkLuaTilescript".to_string(),
            label: "tilescript.lua".to_string(),
            workspace_path: "~/.config/tilescript/.sdk/lua/tilescript.lua".to_string(),
            runtime_path: "/playground/.sdk/lua/tilescript.lua".to_string(),
            include_path: workspace_root_path(template_root)
                .join("packages/sdk/lua/tilescript.lua")
                .to_string_lossy()
                .replace('\\', "/"),
            language: "lua",
...
fn build_spec_with_language(
    ...
    let workspace_path = format!("~/.config/tilescript/{relative}");
