...
async function withLuaEngine<T>(
  ...
  try {
    engine.global.set("__tilescript_sdk_source", sdkSource);
    await engine.doString(`
      local tilescript = assert(load(__tilescript_sdk_source, "@tilescript-sdk"))()
      package.preload["tilescript"] = function()
        return tilescript
      end
    `);
...
async function compileFennelToLua(
  ...
  return withLuaEngine("", async (engine) => {
    engine.global.set("__tilescript_fennel_compiler_source", compilerSource);
    engine.global.set("__tilescript_fennel_source", source);
    engine.global.set("__tilescript_fennel_chunk_name", chunkName);

    return await engine.doString(`
      local fennel = assert(load(__tilescript_fennel_compiler_source, "@fennel-compiler"))()
      return fennel.compileString(__tilescript_fennel_source, {
        filename = __tilescript_fennel_chunk_name,
      })
    `);
  });
}
