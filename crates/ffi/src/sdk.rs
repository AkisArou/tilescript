...
fn remove_legacy_node_modules_link(config_root: &Path) -> Result<bool, FfiError> {
    let legacy_sdk = config_root.join("node_modules/@tilescript/sdk");
    let legacy_scope = config_root.join("node_modules/@tilescript");
...
mod tests {
    ...
    fn sync_sdk_support_writes_managed_sdk_files() {
        ...
        assert!(root.join(".sdk/lua/tilescript.lua").exists());
