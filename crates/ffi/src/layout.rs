use crate::runtime_types::{
    LayoutRuntimeState, LayoutRuntimeStatus, TilescriptRuntimeHandle, WindowGeometryEntry,
};
...
pub fn layout_runtime_status(handle: &mut TilescriptRuntimeHandle) -> LayoutRuntimeStatus {
    let Some(layout_runtime) = handle.layout_runtime.as_mut() else {
...
pub fn layout_runtime_placement(handle: &mut TilescriptRuntimeHandle) -> Vec<PlacementGeometry> {
...
pub fn layout_close_focus_candidate(
    ...
    let window_id = tilescript_core::WindowId::from(window_id.to_string());
...
mod tests {
    ...
    use crate::runtime_types::{LayoutRuntimeState, TilescriptRuntimeHandle};
    use tilescript_config::model::{Config, LayoutRule};
    use tilescript_core::wm::WmModel;
    use tilescript_core::{OutputId, WindowId, WorkspaceId};
    use tilescript_layout_runtime::{LayoutRuntimePaths, LayoutRuntimeService};
    ...
    fn layout_focus_candidate_persists_scene_focus_tree_on_model() {
        ...
        let mut handle = TilescriptRuntimeHandle {
            model,
            layout_runtime: Some(LayoutRuntimeState { config_path: config_path.clone(), service }),
        };
